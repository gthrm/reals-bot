const { DatabaseClient } = require('./database.utils');
...
class UsageService {
  ...
  calculateAudioCost(durationSeconds) {
  ...
  async canUseService(userId, serviceType, duration = null, fileSize = null) {
    try {
      // Ensure user exists
      ...
      switch (serviceType) {
        ...
        case 'audio':
          ...
          break;
        default:
          return { canUse: false, error: 'Invalid service type' };
      }
  ...
  async useService(userId, serviceType, duration = null, fileSize = null) {
    try {
      // Ensure user exists
      await this.ensureUser(userId);
  ...
  async getUserInfo(userId) {
    try {
      ...
      return {
        fedorcoins: parseFloat(user.fedorcoins),
        freeRequests: {
          text: user.free_text_requests,
          image: user.free_image_requests,
          audio: user.free_audio_requests,
        },
        totalRequests: {
          text: user.total_text_requests,
          image: user.total_image_requests,
          audio: user.total_audio_requests,
        },
      };
    } catch (error) {
  ...
  async addFedorCoins(userId, amount, description = 'Balance top-up') {
    try {
      await this.ensureUser(userId);
...
🎵 Аудио: ${totalRequests.audio}`;
  ...
  formatInsufficientBalanceMessage(serviceType, cost, userInfo) {
    const serviceName = {
